...
export function StoreProvider({ children }) {
  ...
  const loadData = async (branch = branchId) => {
    ...
    try {
      ...
      let allProducts = [];
      for (let c of cats) {
        const pres = await fetch(`${API_BASE}/CategoriesApi/products/${c.id}?branchId=${branch}`);
        const prods = await pres.json();
        allProducts = [
          ...allProducts,
          ...prods.map((p) => ({
      ...
      try {
        const cacheData = { timestamp: Date.now(), data: { categories: formattedCategories, products: allProducts } };
        localStorage.setItem(`${CACHE_KEY}_${branch}`, JSON.stringify(cacheData));
      } catch (e) {
        console.error("Could not write to cache.", e);
      }
    } catch (err) {
  ...
  const removeCartItem = (item) => setCart((prev) => prev.filter((i) => i.id !== item.id));

  const clearCart = () => setCart([]);

  const login = async (staff_no, pin, branch = null) => {
    const res = await fetch(`${API_BASE}/CategoriesApi/staff-branches?employeeId=${staff_no}&pin=${pin}`);
    if (!res.ok) throw new Error("Invalid credentials");
  ...
  const updateStudentBalance = (studentId, delta, outstandingDelta = 0) => {
    setStudents((prev) =>
      prev.map((s) =>
        s.id === studentId
          ? { ...s, balance: parseFloat((s.balance + delta).toFixed(2)), outstanding: parseFloat(((s.outstanding ?? 0) + outstandingDelta).toFixed(2)) }
          : s
      )
    );
    return Promise.resolve(true);
  ...
  const recordTxn = (data) => {
    ...
    setTransactions((prev) => [
      { id: prev.length + 1, ts: Date.now(), staff_id: staff?.id, staff_name: staff?.name || "N/A", student_name: student?.name || "N/A", ...data },
      ...prev,
    ]);
    clearCart();
    return Promise.resolve(true);
  ...
  const value = useMemo(
    ...
    [staff, products, categories, transactions, cart, cartTotal, branchId, branchName]
  );
