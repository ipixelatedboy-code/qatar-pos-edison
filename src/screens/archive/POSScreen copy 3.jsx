...
//     useEffect(() => {
//       refreshData?.(); // Load data on mount (will use cache if available)
//     }, [refreshData]);

//     // Effect to select the first category by default once they are loaded
//     useEffect(() => {
...
//         `${API_BASE}/CategoriesApi/product-by-barcode/${code}?branchId=${branchId}`
//       );
...
//       <div style={{ padding: "8px", fontWeight: "bold", color: "#334155" }}>
...
export default function POSScreen() {
  ...
  const processScan = async (code) => {
    ...
    try {
      const res = await fetch(
        ...
        }/CategoriesApi/product-by-barcode/${code}?branchId=${branchId}`
      );
      if (!res.ok) throw new Error("Product not found");
      const product = await res.json();
      addItemToCart(product);
  ...
  const handlePayment = (route) => {
    if (cart.length === 0) return setToast("Cart is empty.");
    navigate(route);
  };
